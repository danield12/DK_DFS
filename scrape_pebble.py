...
def generate_visualizations(df):
    ...
    # Create label with Wind Arrow
    df['Birdie_Label'] = df['Birdie_Better_Pct'].round(1).astype(str) + "\n" + df['Wind_Arrow']
    pivot_birdie = df.pivot(index="Year_Round", columns="Hole", values="Birdie_Better_Pct")
    pivot_birdie_annot = df.pivot(index="Year_Round", columns="Hole", values="Birdie_Label")

    plt.figure(figsize=(14, 8))
    sns.heatmap(pivot_birdie, cmap="Greens", annot=pivot_birdie_annot, fmt="",
                cbar_kws={'label': 'Birdie or Better %'})
    plt.title("Birdie or Better Percentage by Round & Wind Push Direction")
    plt.xlabel("Hole Number")
    plt.ylabel("Round")
    plt.tight_layout()
    ...
    # Create label with Wind Arrow
    df['Bogey_Label'] = df['Bogey_Worse_Pct'].round(1).astype(str) + "\n" + df['Wind_Arrow']
    pivot_bogey = df.pivot(index="Year_Round", columns="Hole", values="Bogey_Worse_Pct")
    pivot_bogey_annot = df.pivot(index="Year_Round", columns="Hole", values="Bogey_Label")

    plt.figure(figsize=(14, 8))
    sns.heatmap(pivot_bogey, cmap="Reds", annot=pivot_bogey_annot, fmt="",
                cbar_kws={'label': 'Bogey or Worse %'})
    plt.title("Bogey or Worse Percentage by Round & Wind Push Direction")
    plt.xlabel("Hole Number")
    plt.ylabel("Round")
    plt.tight_layout()
